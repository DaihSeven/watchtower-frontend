...
// ✅ Ajustado o schema do Zod para status e desaparecidoDesde
const schema = z.object({
  nome: z.string().min(3, "Nome muito curto"),
  idade: z.coerce.number().min(1, "Idade obrigatória"),
  descricao: z.string().min(5, "Descrição muito curta"),
  ultimaLocalizacao: z.string().optional(),
  desaparecidoDesde: z.string().nonempty("Data obrigatória"),
...
export default function PessoaForm({ editData, onSuccess, onCancelEdit }: Props) {
  ...
  useEffect(() => {
    if (editData) {
      reset({
        nome: editData.nome,
        idade: editData.idade ?? 0,
        descricao: editData.descricao ?? "",
        ultimaLocalizacao: editData.ultimaLocalizacao ?? "",
        desaparecidoDesde: editData.desaparecidoDesde,
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {/* ✅ --- Desaparecido Desde (novo) --- */}
        <motion.div
          initial={{ opacity: 0, x: -10 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.3 }}
        >
          <label htmlFor="desaparecidoDesde" className="block text-base font-medium text-gray-800 mb-2">
            Desaparecido Desde
          </label>
          <motion.input
            id="desaparecidoDesde"
            type="date"
            {...register("desaparecidoDesde")}
            whileFocus={{ scale: 1.01, borderColor: "#3b82f6" }}
            className="w-full px-4 py-3 text-gray-900 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-500 transition-all"
          />
          <AnimatePresence>
            {errors.desaparecidoDesde && (
              <motion.p
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                className="text-red-600 text-sm mt-1 font-medium"
              >
                {errors.desaparecidoDesde.message}
              </motion.p>
            )}
          </AnimatePresence>
        </motion.div>
        ...
        >
          <label htmlFor="status" className="block text-base font-medium text-gray-800 mb-2">
            Status
          </label>
          <motion.select
            id="status"
